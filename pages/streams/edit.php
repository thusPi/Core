			...
			<div class="stream-sub-category" data-sub-category="<?php echo($sub_category); ?>" data-set="false" data-show="false">
				<div class="tile-row flex-column flex-xl-row">
					...
					<div class="col-12 col-xl-4 stream-card-select-wrapper">
						<h3 class="tile-title stream-category-title"><?php \thusPi\Locale\translate("streams.action.{$category_name}.title", true); ?></h3>
						<input type="text" class="stream-card-search" data-type="search" name="streams_<?php echo($sub_category); ?>" placeholder="<?php \thusPi\Locale\translate('action.select', true); ?>">
					</div>
					<script>
						$(document).on('thuspi.page_ready', function() {
							...
							<?php 
								...
								foreach ($cards as $card_name) {
									...
									$card_name_translated = \thusPi\Locale\translate("streams.cards.{$cards_category}.{$card_name}.title");
					...
					<div class="stream-cards col">
						<div class="stream-card tile bg-primary show col-12" id="stream-card-empty" data-stream-card="no_stream_card_selected">
							<div class="tile-row">
								<h3 class="stream-card-title tile-title text-overflow-ellipsis">
									<?php \thusPi\Locale\translate('streams.cards.none_selected.title', true); ?>
								</h3>
							</div>
							<div class="tile-row">
								<div class="stream-card-content">
									<span class="text-muted"><?php \thusPi\Locale\translate('streams.cards.none_selected.content', true); ?><span>
								</div>
							</div>
						</div>
						<?php foreach($cards as $card_name) : ?>
							...
							<div class="stream-card tile bg-primary" data-stream-card="<?php echo($card_name); ?>">
								<div class="tile-row">
									<h3 class="stream-card-title tile-title text-overflow-ellipsis"><?php \thusPi\Locale\translate("streams.card_groups.group.{$card['card_group']}.title", true); ?></h3>
								...
								<div class="tile-row">
									<div class="stream-card-content">
										<?php 
											...
											l("streams.cards.{$cards_category}.{$card_name}.content", true, $card_parameters_html,\thusPi\Locale\translate('generic.error'));
										?>
									</div>
								</div>
							</div>
